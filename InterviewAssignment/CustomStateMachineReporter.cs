...

    {
        ...
        public string StateToString(TState state, string separator = ".")
        {
           ...
           //throw new NotImplementedException();
            ...
            var allState = myStates.ToList();
            var stat = allState.Where(s => s.Id.CompareTo(state) == 0);
            var s0 = stat.ToArray()[0];
            var rtn = s0.Id.ToString();
            while (s0.SuperState != null)
            {
                s0 = s0.SuperState;
                rtn = s0 + separator + rtn;
            }
            return rtn;
